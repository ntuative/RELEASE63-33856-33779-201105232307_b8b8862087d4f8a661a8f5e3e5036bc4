package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_807:IFigurePartSet;
      ...
      private const const_2240:int = 13421772;
      
      private const const_2239:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1960 = Math.max(this.var_1960,_loc6_.colorLayerIndex);
            }
         }
         this.var_458 = this.var_69.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_707 != null && !this.var_1959)
            {
               _loc3_ = this.var_707;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_807)
            {
               switch(this.var_807.clubLevel)
               {
                  case HabboClubLevelEnum.const_58:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc2_.style = HabboIconType.const_721;
                     break;
                  case HabboClubLevelEnum.const_46:
                     _loc2_.style = HabboIconType.const_883;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1386.color = this.const_2240;
         }
         else
         {
            this.var_1386.color = this.const_2239;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_368[_loc6_] + "_" + FigureData.const_141;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_663 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1958)
               {
                  ...
                  this.var_1958 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1959 && _loc2_.colorLayerIndex > 0)
               {
