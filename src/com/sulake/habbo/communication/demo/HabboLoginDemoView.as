package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_615:String = "InitConnection";
       
      
      private var var_160:HabboCommunicationDemo;
      
      private var _window:IWindowContainer;
      
      private var var_137:SSOTicket;
      ...
      {
         this.var_160.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_160 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_160.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_141,HabboWindowStyle.const_47,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_160.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1937,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_160.habboConfiguration)
            {
               if(this.var_160.habboConfiguration.getKey("use.sso","false") == "true")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_160.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_160.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1937,"/");
         ...
         {
            dispatchEvent(new Event(const_615));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_160.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_137.removeEventListener(SSOTicket.const_425,this.onParsedTicketSuccess);
            this.var_137.removeEventListener(SSOTicket.const_319,this.onParsedTicketFailure);
            this.var_137 = null;
         }
         var _loc2_:String = "";
         if(this.var_160.habboConfiguration != null)
         {
            _loc2_ = this.var_160.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_160.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_137 = new SSOTicket(this.var_160.assets,_loc3_,_loc2_,param1);
         this.var_137.addEventListener(SSOTicket.const_425,this.onParsedTicketSuccess);
         this.var_137.addEventListener(SSOTicket.const_319,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_137.ticket);
         Logger.log("Has Facebook: " + this.var_137.isFacebookUser);
         if(this.var_137.isFacebookUser)
         {
            this.var_160.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_160.flashClientUrl = this.var_137.flashClientUrl;
         this.var_160.ssoTicket = this.var_137.ticket;
         this.var_137.removeEventListener(SSOTicket.const_425,this.onParsedTicketSuccess);
         this.var_137.removeEventListener(SSOTicket.const_319,this.onParsedTicketFailure);
         this.var_137 = null;
         dispatchEvent(new Event(const_615));
