package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_2079:IWindowContainer;
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_84.getXmlWindow("ude_help"));
         this.var_2079 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1328 = this.var_2079.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1328)
         {
            this.var_1328.procedure = this.onButtonClick;
         }
         this.var_1327 = this.var_2079.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1327)
         {
            this.var_1327.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_281 >= this.var_63.length - 1)
         {
            this.var_1327.disable();
         }
         else
         {
            this.var_1327.enable();
         }
         if(this.var_281 <= 0)
         {
            this.var_1328.disable();
         }
         else
         {
            this.var_1328.enable();
         }
      }
      ...
      {
         ++this.var_281;
         this.var_281 = Math.min(this.var_281,this.var_63.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_281;
         this.var_281 = Math.max(this.var_281,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this.var_63 = [];
         var _loc1_:int = 1;
         while(this.var_84.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_84.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_84.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_84.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_63[this.var_281][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_63[this.var_281][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_63[this.var_281][1]).value;
      ...
      {
         if(!param1 || !this.var_84 || !this.var_84.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_84.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_84.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_84.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
