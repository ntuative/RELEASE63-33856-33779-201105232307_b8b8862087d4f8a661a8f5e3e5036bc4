package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_2207:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2207 = new RoomThumbnailRenderer(param1);
         this.var_905 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_189 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_189.visible = true;
         Util.hideChildren(this.var_189);
         this.refreshRoomName(this.var_189,this.var_125);
         if(this.var_125.ownerName != "" && this.var_125.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_189,!!this.var_125.event ? this.getEventCtg(this.var_125.categoryId) : this.getRoomCtg(this.var_125.categoryId));
         this.refreshRoomDesc(this.var_189,this.var_125);
         this.refreshExtraCont();
         this.refreshInfo(this.var_189,"trading_allowed",this.var_125.allowTrading);
         this.refreshInfo(this.var_189,"doormode_doorbell",this.var_125.doorMode == 1);
         this.refreshInfo(this.var_189,"doormode_password",this.var_125.doorMode == 2);
         Util.moveChildrenToColumn(this.var_189,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_189.height = Util.getLowestPoint(this.var_189);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_189,this.var_125.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_189.findChildByName("roomowner"));
         if(this.var_125.ownerName != "" && this.var_125.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_189.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_905.refreshTags(_loc1_,this.var_125.tags);
         ...
         this.var_2207.refreshThumbnail(_loc1_,this.var_125.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_223,Tabs.const_287,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
