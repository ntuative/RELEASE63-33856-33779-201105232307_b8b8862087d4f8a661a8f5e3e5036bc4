package com.sulake.habbo.ui.widget.playlisteditor
{
   ...
   public class MusicInventoryGridView
   {
      ...
      private var var_101:MusicInventoryGridItem;
      ...
      public function refresh() : void
      {
         ...
         this.var_579.removeGridItems();
         var _loc1_:Map = this._items;
         var _loc2_:Map = new Map();
         var _loc3_:Array = _loc1_.getKeys();
         this._items = new Map();
         var _loc4_:int = this.var_227.getSongDiskInventorySize();
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            _loc7_ = this.var_227.getSongDiskInventoryDiskId(_loc5_);
            _loc8_ = this.var_227.getSongDiskInventorySongId(_loc5_);
            _loc9_ = this.var_227.getSongInfo(_loc8_);
            _loc10_ = null;
            _loc11_ = null;
            if(_loc9_ != null)
            {
            ...
            this.var_579.addGridItem(_loc12_.window);
      ...
      public function setPreviewIconToPause() : void
      {
         ...
         {
            this.var_101.playButtonState = MusicInventoryGridItem.const_1290;
         }
      }
      ...
      public function setPreviewIconToPlay() : void
      {
         if(this.var_101 != null)
         {
            this.var_101.playButtonState = MusicInventoryGridItem.const_474;
         }
      }
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK || _loc3_)
         {
            if(param2.name == "button_to_playlist" || _loc3_)
            {
               if(this.var_101 != null)
               {
                  this.var_101.deselect();
                  this.stopPreview();
                  this._widget.sendAddToPlayListMessage(this.var_101.diskId);
                  this.var_101 = null;
               }
            }
            else if(param2.name == "button_play_pause")
            {
               if(this.var_101.playButtonState == MusicInventoryGridItem.const_474)
               {
                  this.var_101.playButtonState = MusicInventoryGridItem.const_1219;
                  this._widget.playUserSong(this.var_101.songId);
               }
               else
               {
                  this.stopPreview();
            ...
            {
               _loc4_ = this.var_579.getGridItemIndex(param1.window);
               if(_loc4_ != -1)
               {
                  _loc5_ = this._items.getWithIndex(_loc4_);
                  if(_loc5_ != this.var_101)
                  {
                     if(this.var_101 != null)
                     {
                        this.var_101.deselect();
