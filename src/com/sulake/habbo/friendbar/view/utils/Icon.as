package com.sulake.habbo.friendbar.view.utils
{
   import com.sulake.core.assets.BitmapDataAsset;
   import com.sulake.core.runtime.IDisposable;
   import com.sulake.core.window.components.IBitmapWrapperWindow;
   import flash.display.BitmapData;
   import flash.events.TimerEvent;
   import flash.geom.Point;
   import flash.utils.Timer;
   
   public class Icon implements IDisposable
   {
      
      protected static const const_2202:int = 0;
      
      protected static const const_638:int = 1;
      
      protected static const const_1934:int = 2;
      
      protected static const const_1936:int = 3;
      
      protected static const const_126:int = 4;
      
      protected static const const_637:int = 8;
      
      protected static const const_285:int = 18;
      
      protected static const const_1935:int = 18;
       
      
      private var _disposed:Boolean = false;
      
      private var var_1669:Boolean = false;
      
      protected var var_47:BitmapDataAsset;
      
      protected var var_91:IBitmapWrapperWindow;
      
      private var var_1325:uint;
      
      protected var var_216:Timer;
      
      protected var _frame:int = 0;
      
      private var var_690:Point;
      
      protected var var_881:Boolean = false;
      
      protected var _hover:Boolean = false;
      
      public function Icon()
      {
         this.var_1325 = const_638 | const_637;
         this.var_690 = new Point();
         super();
      }
      
      public function get disposed() : Boolean
      {
         return this._disposed;
      }
      
      public function get disabled() : Boolean
      {
         return this.var_1669;
      }
      
      protected function set image(param1:BitmapDataAsset) : void
      {
         this.var_47 = param1;
         this.redraw();
      }
      
      protected function get image() : BitmapDataAsset
      {
         return this.var_47;
      }
      
      protected function set canvas(param1:IBitmapWrapperWindow) : void
      {
         this.var_91 = param1;
         this.redraw();
      }
      
      protected function get canvas() : IBitmapWrapperWindow
      {
         return this.var_91;
      }
      
      protected function set alignment(param1:uint) : void
      {
         this.var_1325 = param1;
         this.redraw();
      }
      
      protected function get alignment() : uint
      {
         return this.var_1325;
      }
      
      public function dispose() : void
      {
         if(!this._disposed)
         {
            this.toggleTimer(false,0);
            this.image = null;
            this.canvas = null;
            this._disposed = true;
         }
      }
      
      public function notify(param1:Boolean) : void
      {
         this.var_881 = param1;
         if(this.var_881 && this.var_1669)
         {
            this.enable(true);
         }
      }
      
      public function hover(param1:Boolean) : void
      {
         this._hover = param1;
      }
      
      public function enable(param1:Boolean) : void
      {
         this.var_1669 = !param1;
      }
      
      protected function redraw() : void
      {
         var _loc1_:* = null;
         if(this.var_91 && !this.var_91.disposed)
         {
            if(!this.var_91.bitmap)
            {
               this.var_91.bitmap = new BitmapData(this.var_91.width,this.var_91.height,true,0);
            }
            else
            {
               this.var_91.bitmap.fillRect(this.var_91.bitmap.rect,0);
            }
            if(this.var_47 && !this.var_47.disposed)
            {
               this.var_690.x = this.var_690.y = 0;
               _loc1_ = this.var_47.content as BitmapData;
               switch(this.var_1325 & const_1936)
               {
                  case const_638:
                     this.var_690.x = this.var_91.bitmap.width / 2 - _loc1_.width / 2;
                     break;
                  case const_1934:
                     this.var_690.x = this.var_91.bitmap.width - _loc1_.width;
               }
               switch(this.var_1325 & const_1935)
               {
                  case const_637:
                     this.var_690.y = this.var_91.bitmap.height / 2 - _loc1_.height / 2;
                     break;
                  case const_285:
                     this.var_690.y = this.var_91.bitmap.height - _loc1_.height;
               }
               this.var_91.bitmap.copyPixels(_loc1_,_loc1_.rect,this.var_690);
               this.var_91.invalidate();
            }
         }
      }
      
      protected function toggleTimer(param1:Boolean, param2:int) : void
      {
         if(param1)
         {
            if(!this.var_216)
            {
               this.var_216 = new Timer(param2,0);
               this.var_216.addEventListener(TimerEvent.TIMER,this.onTimerEvent);
               this.var_216.start();
               this.onTimerEvent(null);
            }
            this.var_216.delay = param2;
         }
         else
         {
            this._frame = 0;
            if(this.var_216)
            {
               this.var_216.reset();
               this.var_216.removeEventListener(TimerEvent.TIMER,this.onTimerEvent);
               this.var_216 = null;
            }
         }
      }
      
      protected function onTimerEvent(param1:TimerEvent) : void
      {
      }
   }
}
