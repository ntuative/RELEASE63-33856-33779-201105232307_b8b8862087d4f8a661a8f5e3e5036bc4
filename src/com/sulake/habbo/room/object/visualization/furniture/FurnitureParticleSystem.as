package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_105:FurnitureParticleSystemEmitter;
      ...
      private var var_1317:Number = 1.0;
      
      private var var_687:BitmapData;
      
      private var var_842:ColorTransform;
      
      private var var_1589:ColorTransform;
      
      private var var_2046:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_842 = new ColorTransform();
         this.var_842.alphaMultiplier = 1;
         this.var_1589 = new ColorTransform();
         this.var_2046 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_105 = this.var_1316[param1];
         this.var_1059 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_2045 >= 0)
         {
            this.var_279 = this._visualization.getSprite(this.var_2045);
            if(this.var_279 && this.var_279.asset)
            {
               ...
               if(this.var_91 == null)
               {
                  this.var_91 = this.var_279.asset.clone();
                  if(this.var_842.alphaMultiplier != 1)
                  {
                     this.var_687 = new BitmapData(this.var_91.width,this.var_91.height,true,4278190080);
                  }
               }
               this.var_2715 = -this.var_279.offsetX;
               this.var_2714 = -this.var_279.offsetY;
               this.var_279.asset = this.var_91;
            }
            if(this.var_91)
            {
               this.var_91.fillRect(this.var_91.rect,4278190080);
            }
            if(this.var_687)
            {
      ...
      {
         if(this.var_105 && this.var_105.roomObjectSpriteId == param3)
         {
            return this.var_105.y * this.var_1317;
         }
         return 0;
      }
      ...
      public function controlsSprite(param1:int) : Boolean
      {
         if(this.var_105)
         {
            return this.var_105.roomObjectSpriteId == param1;
         }
         return false;
      }
      ...
      {
         if(!this.var_105 || !this.var_279)
         {
            return;
         }
         if(this.var_91 && this.var_279.asset != this.var_91)
         {
            this.var_279.asset = this.var_91;
         }
         if(this.var_1059)
         {
            if(this.var_105.roomObjectSpriteId >= 0)
            {
               this._visualization.getSprite(this.var_105.roomObjectSpriteId).visible = false;
            }
         }
      }
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1059 && this.var_105.hasIgnited)
         {
            this.var_1059 = true;
         }
         _loc8_ = this.var_1194 * this.var_1317;
         this.var_105.update();
         if(this.var_1059)
         {
            if(this.var_105.roomObjectSpriteId >= 0)
            ...
            {
               this.var_91.draw(this.var_687,this.var_2046,this.var_842,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_105.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2715 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1317;
               _loc4_ = this.var_2714 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1317;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1589.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_91.draw(_loc7_,this._translationMatrix,this.var_1589,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
                     _loc10_ = new Point(_loc3_ + _loc6_.offsetX,_loc4_ + _loc6_.offsetY);
                     this.var_91.copyPixels(_loc7_,_loc7_.rect,_loc10_,null,null,true);
                  }
               }
               else
               {
                  _loc5_ = new Rectangle(_loc3_ - 1,_loc4_ - 1,2,2);
                  this.var_91.fillRect(_loc5_,4294967295);
               }
            }
            this.var_91.unlock();
         }
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2850 = parseInt(param1.@size);
         this.var_2045 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1194 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1317 = this.var_2850 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_842.alphaMultiplier = this._blend;
