package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1581:String = "asset-libraries";
      
      private static const const_1577:String = "library";
      
      private static const const_1580:String = "service-libraries";
      
      private static const const_1582:String = "library";
      
      private static const const_1579:String = "component-libraries";
      
      private static const const_1583:String = "library";
      
      private static const const_1584:String = "error_data";
      
      private static const const_1576:String = "error_cat";
      
      private static const const_1578:String = "error_desc";
       
      
      private var var_391:LibraryLoaderQueue;
      
      private var var_846:IEventDispatcher;
      
      private var var_558:uint;
      
      private var var_392:Function;
      
      private var var_212:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1159:
               ...
               this.var_392 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_875:
               debug("Core; using complex frame update handler");
               this.var_392 = this.complexFrameUpdateHandler;
               break;
            case Core.const_763:
               debug("Core; using profiler frame update handler");
               this.var_392 = this.profilerFrameUpdateHandler;
               this.var_212 = new Profiler(this);
               attachComponent(this.var_212,[new IIDProfiler()]);
               var_407.addChild(new ProfilerViewer(this.var_212));
               break;
            case Core.const_544:
               debug("Core; using experimental frame update handler");
               this.var_392 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_392 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_391.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_558 - this.getNumberOfFilesPending();
      }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1584,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1576,String(param3));
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_846 = loadingEventDelegate;
         if(this.var_391 == null)
         {
            this.var_391 = new LibraryLoaderQueue(_debug);
         }
         try
         {
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_362,e);
         }
         try
         {
            node = config.child(const_1580)[0];
            if(node != null)
            {
               list = node.child(const_1582);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_362,e);
         }
         try
         {
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_362,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1772);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_846)
         {
            this.var_846.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_846)
         {
            ...
            this.var_846.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_846)
                  {
                     this.var_846.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_558 - this.var_391.length,this.var_558,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_212 ? 0 : this.var_1596 & 0;
         if(_loc3_ == Core.const_544)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_392(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_104)
         {
            ...
            {
               if(this.var_688[priority] < priority)
               {
                  ++this.var_688[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_104)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_212.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_212)
            {
               detachComponent(this.var_212);
               this.var_212.dispose();
               this.var_212 = null;
            }
            switch(this.var_1596 & 0)
            {
               case Core.const_1159:
                  this.var_392 = this.simpleFrameUpdateHandler;
                  break;
